...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        int res = fscanf(file, " %u %u %u ", &k_year, &k_month, &k_day); // fscanf() devuelve el numero de valores que han sido leidos del fichero y almacenados en las variables.
        if (res != 3) {
        ...
        // Ahora tengo que completar cada medicion en el lugar del arreglo correspondiente
        
        array[k_year-FST_YEAR][(k_month+11)-MONTHS][k_day-FST_DAY] = weather;
