...
Weather weather_from_file(FILE* file)
{
    Weather weather;
    /* Completar aqui */
    /* entiendo que tengo que leer el archivo y guardarlo en las variables correspondientes */

    if (AMOUNT_OF_WEATHER_VARS != fscanf(file, "%d %d %d %u %u %u\n", &weather._average_temp, &weather._max_temp, &weather._min_temp, &weather._pressure, &weather._moisture, &weather._rainfall )){
      fprintf(stdout,"Invalid read.\n");
    }
